...
// 기존 src/components/specific/group/GroupSidebar.tsx
...
// 수연 src/components/specific/group/GroupSidebar.tsx
...
export default function GroupSidebar({
  ...
  const entries = useMemo(
    () => Object.entries(weekData) as [string, number][],
    [weekData]
  );

  // ✅ 1) TOP3 + TOP3를 기반으로 한 후보(candidates) + 황금시간 한 번에 계산
  const { candidates, top3, golden } = useMemo(() => {
    if (entries.length === 0 || memberCount <= 0) {
      return {
    ...
    const sorted = [...entries].sort((a, b) => b[1] - a[1]); // count 내림차순
    const top3Entries = sorted.slice(0, 3);

    const top3: TopItem[] = top3Entries.map(([key, count], idx) => {
      const [day, hour] = key.split("-").map(Number);
      const safeCount = Math.max(0, Math.min(memberCount, count));
      const percent = Math.round((safeCount / memberCount) * 100);
    ...
    // ✅ 후보(candidates)는 “최적 시간 TOP3”만 사용
    const candidates: CandidateItem[] = top3Entries.map(([key, count]) => {
      const [day, hour] = key.split("-").map(Number);
      const safeCount = Math.max(0, Math.min(memberCount, count));
    ...
    const safeBestCount = Math.max(0, Math.min(memberCount, bestCount));
    const golden = {
      time: `${WEEKDAYS[bestDay]}요일 ${bestHour}:00~${bestHour + 1}:00`,
      count: safeBestCount,
      percent: Math.round((safeBestCount / memberCount) * 100),
    };

    return { candidates, top3, golden };
  }, [entries, memberCount]);

  // ✅ 2) 시간 후보/Top3를 전역 스토어(zustand)에 저장 → 룰렛·투표에서 사용
  useEffect(() => {
    useTimeDecisionStore.getState().setDecisionData({
      top3,
      candidates, // 🔥 이제 후보 = 최적 시간 TOP3
      // 지금은 참여자 이름 리스트는 따로 안 쓰므로 비워둠
      participants: [],
    });
