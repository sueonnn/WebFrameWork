// - 부모에서 상위 N개만 잘라서 전달 (여기서는 top3 사용 가정)
// - 각 역에 대해: 이름, 주소, 중심까지 거리, 예상 시간 표시
// - 역이 없을 경우 안내 문구 출력
...
export const GroupPlaceStationList = ({ topStations }: Props) => {
  ...
  const openCafes = (s: Station) => {
    nav(`/stations/cafes?name=${encodeURIComponent(s.name)}&lat=${s.pos.lat}&lng=${s.pos.lng}`);
  };
        ...
        {topStations.map((s, idx) => (
          ...
          >
            ...
            <div className="mt-2 text-sm">
              ...
              {/* 평균 이동 거리 기반 대략적인 시간 (도보 80m/분 가정) */}
